...
class PUNEmbeddings(nn.Module):
    def __init__(self, config: Config):
        super().__init__()
        self.feat_embeddings = nn.Embedding(config.input_size, config.embed_size)
        self.field_embeddings = nn.Embedding(config.num_fields, config.embed_size)
        self.type_embeddings = nn.Embedding(config.num_feat_types, config.embed_size)
        fan_in, fan_out = config.num_fields, config.embed_size
        # TODO: better initialization
        std = math.sqrt(2. / float(fan_in + fan_out))
        with torch.no_grad():
            init.normal_(self.feat_embeddings.weight, std=std)
            init.normal_(self.field_embeddings.weight, std=std)
            init.normal_(self.type_embeddings.weight, std=std)
        self.embed_norm = config.embed_norm
        if self.embed_norm:
            self.layer_norm = nn.LayerNorm(config.embed_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.embed_dropout_rate)

    def forward(self, input_ids, field_ids, type_ids):
        input_embeds = self.feat_embeddings(input_ids)
        field_embeds = self.field_embeddings(field_ids)
        # type_embeds = self.type_embeddings(type_ids)
        embeddings = input_embeds + field_embeds# + type_embeds
        if self.embed_norm:
            embeddings = self.layer_norm(embeddings)
        embeddings = self.dropout(embeddings)
        return embeddings
...
class InteractionLayerV4(nn.Module):
    def __init__(self, config: Config):
        super(InteractionLayerV4, self).__init__()

        # elementwise product after projection
        self.kernel = nn.Parameter(torch.empty(config.num_fields, config.hidden_size, config.hidden_size,
                                               dtype=torch.float32))
        self.bias = nn.Parameter(torch.empty(config.num_fields, dtype=torch.float32))
        # with torch.no_grad():
        # nn.init.xavier_uniform_()
        nn.init.xavier_normal_(self.kernel.data, gain=1)
        # fan_in, fan_out = config.hidden_size * config.num_fields, config.hidden_size * config.hidden_size
        # nn.init.normal_(self.kernel.data, std=math.sqrt(2.0 / float(fan_in + fan_out)))
        nn.init.zeros_(self.bias)
        ...
        # self.bias = nn.Parameter(torch.empty(config.num_fields, dtype=torch.float32))
        # with torch.no_grad():
        #     fan_in, fan_out = config.hidden_size * config.hidden_size, config.hidden_size
        #     nn.init.normal_(self.kernel.data, std=math.sqrt(2.0 / float(fan_in + fan_out)))
        #     nn.init.zeros_(self.bias)

        # self.self_attn = SelfAttention(config, output_attention_probs=True)
        self.attn = Attention(config, output_attention_probs=True)
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        self.layer_norm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.norm_first = config.norm_first
        self.dropout = nn.Dropout(config.prod_dropout_rate)

    def forward(self, hidden_states):
        """
        :param hidden_states: B * N * E
        :return: B * N * E
        """
        input_tensor = hidden_states
        if self.norm_first:
        ...
        # B * N * E
        pk = torch.bmm(hidden_states.permute((1, 0, 2)), self.kernel).permute((1, 0, 2))
        # B * 1 * E
        # q, attn_weights = self.self_attn(hidden_states)
        q = hidden_states.sum(dim=1, keepdim=True)
        hidden_states = pk * q + self.bias.unsqueeze(0).unsqueeze(-1)
        # hidden_states, _ = self.self_attn(hidden_states)
        # hidden_states *= hidden_states.sum(dim=1, keepdim=True)

        # # interact before agg
        # # B * N * E
        # query = torch.bmm(hidden_states.permute((1, 0, 2)), self.kernel).permute((1, 0, 2))
        # # B * N * N * E
        # key = query.unsqueeze(2) * hidden_states.unsqueeze(1)
        # hidden_states, _ = self.attn(query, key)
...
class ProductLayerV4(nn.Module):
    def __init__(self, config: Config):
        super(ProductLayerV4, self).__init__()
        self.interaction_layer = InteractionLayerV4(config)
        self.intermediate_layer = IntermediateLayer(config)

    def forward(self, hidden_states):
        self_output = self.interaction_layer(hidden_states)
        inter_output = self.intermediate_layer(self_output)
        return inter_output


class InteractionLayerV5(nn.Module):
    def __init__(self, config: Config, c_in=1, c_out=1):
        super(InteractionLayerV5, self).__init__()
        ...
        nn.init.zeros_(self.bias)
        # TODO identity init
        # fan_in = c_in * hidden_size * hidden_size
        # fan_out = c_out * hidden_size * hidden_size
        # self.kernel.data.copy_(torch.eye(hidden_size, dtype=torch.float32) * math.sqrt(2 / (fan_in + fan_out)))
        # self.bias.data.copy_(torch.zeros(c_out, dtype=torch.float32))
        self.agg_type = config.agg_type
        if self.agg_type == 'attn':
            self.self_attn = SelfAttention(config, output_attention_probs=True)
    ...
    def forward(self, hidden_states):
        """
        :param hidden_states: B * N * Ci * E
        :return: B * N * Co * E
        """
        c_out, c_in, N, E, _ = self.kernel.shape

        # p: B * N * c_in * E -> (N * c_in) * B * E
        # k: c_out * c_in * N * E * E -> (N * c_in) * E * (c_out * E)
        # p @ k: -> N * c_in * B * c_out * E -> (B * N * E) * c_out * c_in
        pk = torch.bmm(hidden_states.permute((1, 2, 0, 3)).reshape((N * c_in, -1, E)),
                       self.kernel.permute((2, 1, 3, 0, 4)).reshape((N * c_in, E, c_out * E))). \
            reshape((N, c_in, -1, c_out, E)).permute((2, 0, 4, 3, 1)).reshape((-1, c_out, c_in))

        if self.agg_type == 'attn':
            if self.layer_norm is not None and self.norm_first:
                q = self.layer_norm(hidden_states)
            else:
                q = hidden_states
            # (B * c_in) * N * E
            q = q.permute((0, 2, 1, 3)).reshape((-1, N, E))
...
class ProductLayerV5(nn.Module):
    def __init__(self, config: Config, c_in=1, c_out=1):
        super(ProductLayerV5, self).__init__()
        self.interaction_layer = InteractionLayerV5(config, c_in=c_in, c_out=c_out)
        self.intermediate_layer = IntermediateLayer(config)

    def forward(self, hidden_states):
        agg_output = self.interaction_layer(hidden_states)
        inter_output = self.intermediate_layer(agg_output)
        return inter_output


class InteractionLayerV6(nn.Module):
    def __init__(self, config: Config, c_in=1, c_out=1):
        super(InteractionLayerV6, self).__init__()
    ...
    def forward(self, hidden_states):
        ...
        # (B * c_in) * N * E
        hidden_states = hidden_states.permute((0, 2, 1, 3)).reshape((-1, N, E))
        hidden_states, attn_weights = self.self_attn(hidden_states)

        # p: B * N * c_in * E -> (N * c_in) * B * E
        # k: c_out * c_in * N * E * E -> (N * c_in) * E * (c_out * E)
        # p @ k: -> N * c_in * B * c_out * E
        pk = torch.bmm(hidden_states.permute((1, 2, 0, 3)).reshape((N * c_in, -1, E)),
                       self.kernel.permute((2, 1, 3, 0, 4)).reshape((N * c_in, E, c_out * E))). \
            reshape((N, c_in, -1, c_out, E))

        if self.agg_type == 'none':
            # -> (B * N * E) * c_out * c_in
            pk = pk.permute((2, 0, 4, 3, 1)).reshape((-1, c_out, c_in))
            # (B * c_in) * N * E
            q = hidden_states.permute((0, 2, 1, 3)).reshape((-1, N, E))
            # (B * c_in) * N * E
            q, attn_weights = self.self_attn(q)
            # (B * N * E) * c_in * 1
            q = q.reshape((-1, c_in, N, E)).permute((0, 2, 3, 1)).reshape((-1, c_in, 1))
            ...
            # (B * N * E) * c_out -> B * N * c_out * E
            hidden_states = torch.bmm(pk, q).reshape((-1, N, E, c_out)).permute((0, 1, 3, 2))
        else:
            # -> (B * E) * (N * c_out) * c_in
            pk = pk.permute((2, 4, 0, 3, 1)).reshape((-1, N * c_out, c_in))

            if self.agg_type == 'sum':
                q = hidden_states.sum(dim=1)
            else:
                # B * c_in * E
                q = hidden_states.mean(dim=1)
            # (B * E) * c_in * 1
            q = q.permute((0, 2, 1)).reshape((-1, c_in, 1))

            # (B * E) * (N * c_out) * 1 -> B * N * c_out * E
            hidden_states = torch.bmm(pk, q).reshape((-1, E, N, c_out)).permute((0, 2, 3, 1))

        # N * c_out -> 1 * N * c_out * E
        hidden_states += self.bias.unsqueeze(0).unsqueeze(-1)
...
class ProductLayerV6(nn.Module):
    def __init__(self, config: Config, c_in=1, c_out=1):
        super(ProductLayerV6, self).__init__()
        self.interaction_layer = InteractionLayerV6(config, c_in=c_in, c_out=c_out)
        self.intermediate_layer = IntermediateLayer(config)
